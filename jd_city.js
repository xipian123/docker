...
let inviteCodes = ["RtGKzOWnF1unLIXLH9M01-fHxIFax5wDJVDIVnfZLQrFqk-xrQ@RtGKzb6hF1nxeoWTFdVm0b7X0LHaTiAyeFufphTAOJum3KF7qA@RtGKz7qiFA-qetbIH4U0hf7UtHmZc7t2zHkyC6lPBy42SaFVhg@RtGKzOunF1ymedCTFYE70gtbIfyRxDm-xE79gxg6ffV04Ie_bA@RtGKsojeNlPKLd_BUK5WmnzKDcF_5PSDGRj9dUHLGXcU28GC"];
!(async () => {
  if (!cookiesArr[0]) {
    $.msg(
      $.name,
      ...
      for (let i = 0; i < $.newShareCodes.length; ++i) {
        ...
        if (res && res["data"] && res["data"]["bizCode"] === 0) {
          ...
          if (
            ...
            console.log(
              `助力 【${$.newShareCodes[i]}】:${res.data.result.toasts[0].msg}`
            );
          }
        }
        if (
          ...
          if (safeGet(data)) {
            ...
            if (data.code === 0) {
              if (data.data && data["data"]["bizCode"] === 0) {
                if (flag)
                  console.log(
                    `\n【京东账号${$.index}（${$.UserName}）的${
                      $.name
                    }好友互助码】${
                      data.data && data.data.result.userActBaseInfo.inviteId
                    }\n`
                  );
                for (let vo of (data.data.result &&
                  data.data.result.mainInfos) ||
                  []) {
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data["data"]["bizCode"] === 0) {
              console.log(
                `获得 ${data.data.result.currentTimeCash} 元，共计 ${data.data.result.totalCash} 元`
              );
            }
          }
        }
...
function getInviteInfo() {
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("city_masterMainData", body),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
